...
st.sidebar.selectbox("What would you like to test?", ("Prompt Injection", "Prompt Leaking", "Jailbreak"))
st.sidebar.radio(
    "Choose a LLM model",
...
if uploaded_file:
    ...
    try:
        if ext == "csv":
            ...
            txt = io.StringIO(uploaded_file.getvalue().decode("utf-8")).read()
            st.session_state.chat_history.append({"role": "uploader", "message": txt, "timestamp": datetime.now().strftime('%H:%M')})
        elif ext == "xlsx":
            st.dataframe(pd.read_excel(uploaded_file, engine='openpyxl'))
        elif ext == "json":
